...
class UMLGenerator:
    def __init__(self, module_dirs: Iterable[str], recursion_level: int = 1) -> None:
    ...
    def __call__(self, *args: Any, **kwds: Any) -> Any:
        for module_path in self._module_dirs:
    ...
    @staticmethod
    def _generate_command(project_name: str, imports: Iterable[str]) -> str:
        return f"pyreverse -o png -p {project_name} -d documentation/uml --colorized {unpack(imports)}"
    ...
    def _get_imports_from_submodules(self, module_path: str) -> Iterable[str]:
        ...
        for sub_module_path in find_submodules(module_path):
            for python_file_name in glob.glob(sub_module_path + "/*.py"):
                ...
                imports.extend(
                    self._get_recursive_imports(python_file_name, self._recursion_level)
                )
        return imports
